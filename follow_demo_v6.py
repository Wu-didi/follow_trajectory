...
class VehicleTrajectoryFollower:
    ...
    def find_closest_point_index(self, current_lat, current_lon, trajectory=None):
        """
        找到距离当前车辆位置最近的轨迹点索引
    ...
    def calculate_turn_angle(self, current_position, current_heading):
        ...
        target_index = min(self.closest_index + self.offset_target_index, len(self.current_trajectory) - 1)  # 防止超出范围
    ...
    def check_trajectory_safe(self, trajectory, obstacles, safe_distance, which_trajectory = 0):
        """
        检查给定轨迹是否安全
        ...
        for obstacle in obstacles:
            ...
            if abs(obstacle_y + which_trajectory) <= 3:
                print(obstacle_y + which_trajectory)
                return False
            # obstacle_lon, obstacle_lat = utm_to_latlon(obstacle_x, obstacle_y)
            # closest_point_idx = self.find_closest_point_index(obstacle_lat, obstacle_lon, trajectory=trajectory)
    ...
    def check_and_switch_trajectory(self, obstacles, safe_distance=4):
        ...
        if self.is_using_alternate:
            main_safe = self.check_trajectory_safe(self.main_trajectory, obstacles, safe_distance, which_trajectory = -4 )
            alternate_safe = self.check_trajectory_safe(self.alternate_trajectory, obstacles, safe_distance, which_trajectory = 0)
        else:
            main_safe = self.check_trajectory_safe(self.main_trajectory, obstacles, safe_distance, which_trajectory = 0 )
            alternate_safe = self.check_trajectory_safe(self.alternate_trajectory, obstacles, safe_distance, which_trajectory = -4)
            ...
            if can_use.ego_lon is not None and can_use.ego_lat is not None:
                ...
                with shared_data["lock"]:
                    if shared_data["frame"]['perception_result'] is not None:
                        ...
                        for one_obstacle in last_perception_frame:
                            ...
                            if obstacle_x is not None and obstacle_y is not None and abs(obstacle_y) < 4 and abs(obstacle_x) < 8:
                                ...
                                obstacles_memoryBank.append([obstacle_x, obstacle_y])
                # 更新障碍物列表
                ...
                if follower.should_stop:
                    ...
                    desired_speed, desired_acc = follower.calculate_speedAndacc(
                        turn_angle, (can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw), can_use.ego_v
                    )
...
def main():
    # 使用示例
    camera_index = 0
    main_trajectory_csv = '/home/nvidia/vcii/follow_trajectory/collect_trajectory/processed_haima-1116-left.csv'
    alternate_trajectory_csv = '/home/nvidia/vcii/follow_trajectory/collect_trajectory/processed_haima-1116-right.csv'
