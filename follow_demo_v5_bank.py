...
class VehicleTrajectoryFollower:
    ...
    def find_closest_point_index(self, current_lat, current_lon, trajectory=None):
        ...
        if self.closest_index == 0:
            ...
            max_bound = 200
        print(max(self.closest_index,0),min(self.closest_index+max_bound, len(trajectory)-1))
        for i, (lon, lat, _) in enumerate(trajectory[self.closest_index:max(self.closest_index+max_bound,len(trajectory))]):  # 经度在前，纬度在后
            distance = self.calculate_distance(current_lat, current_lon, lat, lon)
            if distance < min_distance:
                min_distance = distance
                closest_index = i
    ...
    def smooth_turn_angle(self, turn_angle):
        ...
        if angle_difference > self.max_turn_rate:
            update_turn_angle = self.previous_turn_angle + 2
        ...
        if mod_AE == 1 and mod_666 == 1:
            if can_use.ego_lon is not None and can_use.ego_lat is not None:
                ...
                with shared_data["lock"]:
                    if shared_data["frame"]['perception_result'] is not None:
                        ...
                        for one_obstacle in last_perception_frame:
                            ...
                            if obstacle_x is not None and obstacle_y is not None and abs(obstacle_y) < 4 and abs(obstacle_x) < 15:
                ...
                for one in obstacles_list_xy:
                    if  abs(one[0]) < 50 and abs(one[1]) < 6:
                        obstacle_detected = True
                        break
                    else:
                        obstacle_detected = False
                # 检查并切换轨迹或停车
                follower.check_and_switch_trajectory(obstacles_list, safe_distance=4)

                if follower.should_stop:
                    # 如果需要停车，发送停车指令
                    print("车辆停止")
                    new_frame = [0, 0, -3]  # 设置速度为0，加速度为-1表示制动
                else:
                    # 正常计算控制指令
                    if obstacle_detected:
                        turn_angle = follower.calculate_turn_angle(
                            (can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw), can_use.ego_yaw, offset_target_index=10
                        )
                        turn_angle /= 2
                    else:
                        turn_angle = follower.calculate_turn_angle(
...
def main():
    # 使用示例
    camera_index = 0
    main_trajectory_csv = '/home/nvidia/vcii/follow_trajectory/collect_trajectory/processed_haima-1119-left.csv'
