...
mod_666 = 0
mod_AE = 0
class angel_with_grade :
# Define the input signal
    UpLvTorqCmd = 5.0  # Replace with actual input value if available

# Initialize the feedback variable and output
    Tor_fnal_with_tm = 0.0
    feedback_value = 0.0  # Assuming a feedback signal from delay block (1/z)

# Helper functions representing operations in Simulink blocks
    def add(a, b):
        return a + b
...
# Main function representing the logic in the Simulink model
    def calculate_torque(UpLvTorqCmd, feedback_value):
        global Tor_fnal_with_tm  # Declare as global to modify outside function scope
    
    # Step 1: Perform equality check (== block)
        condition_eq = equal(UpLvTorqCmd, feedback_value)
    
    # Step 2: Perform addition and subtraction operations
        result_add = add(UpLvTorqCmd, 2)
        result_subtract = subtract(UpLvTorqCmd, 2)
    
    # Step 3: Summation block (Σ) that might add multiple signals (based on structure)
        result_sum = add(result_add, result_subtract)
    
    # Step 4: Absolute value block
        abs_result = absolute(result_subtract)
    
    # Step 5: Comparison operations for conditional control
        condition_gt_0 = greater_than(result_sum, 0)
        condition_gt_2 = greater_than(abs_result, 2)
    
    # Step 6: Final output based on conditions
    if condition_gt_0 and condition_gt_2:
        Tor_fnal_with_tm = result_sum
    else:
        Tor_fnal_with_tm = 0  # Default or alternative output value if conditions not met

    # Step 7: Update feedback value with delay (feedback loop through 1/z)
    feedback_value = Tor_fnal_with_tm

# Run the model calculation
calculate_torque(UpLvTorqCmd, feedback_value)
...
def send_frame(shared_data,can_use):
    ...
    while True:
        ...
        if last_frame != None:
            ...
            if last_frame[1]*WHEEL_FACTOR>380:
                last_frame[1] = 380
            elif last_frame[1]*WHEEL_FACTOR<-380:
                last_frame[1] = -380
            else:
...
#                last_frame[1] = last_frame[1]*WHEEL_FACTOR
            ...
            # if mod_AE == 3:
                # print("+++++++++++++++++++++++++++++++++++++")
                # can_use.publish_planner_ation(action=0, id=0x0AE, action_type="angle", mod=mod_AE, enable=1)
                # time.sleep(2)
                # can_use.publish_planner_ation(action=0, id=0x0AE, action_type="angle", mod=1, enable=1)
            # else: 
                # can_use.publish_planner_ation(action = last_frame[1], id=0x0AE, action_type="angle", mod=1, enable=1)
            can_use.publish_planner_ation(action = last_frame[1], id=0x666, action_type="acc", mod=1, enable=1)
