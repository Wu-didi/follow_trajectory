...
class VehicleTrajectoryFollower:
    def __init__(self, trajectory_csv, target_index=30):
        ...
        self.wheelbase = 2.85
        self.target_index = 0
        self.previous_turn_angle = None  # 用于存储上一帧的转向角
    ...
    def find_closest_point_index(self, current_lat, current_lon):
        ...
        for i, (lon, lat, _) in enumerate(self.trajectory_points[self.closest_index:min(self.closest_index + 200, len(self.trajectory_points))]):
    ...
    def calculate_turn_angle(self, current_position, current_heading):
        ...
        closest_index = self.find_closest_point_index(front_lat, front_lon)
        self.closest_index = closest_index
        target_index = min(closest_index + self.offset_target_index, len(self.trajectory_points) - 1)
        next_lon, next_lat, _ = self.trajectory_points[target_index]
        ...
        # 判断是否要使用上一帧的 turn_angle 值
        if self.previous_turn_angle is not None:
            angle_diff = abs(turn_angle - self.previous_turn_angle)
            if angle_diff > 6:
                # 如果差值大于30度，将当前 turn_angle 替换为上一帧的角度值
                turn_angle = self.previous_turn_angle
            else:
                # 如果差值小于等于30度，更新上一帧的角度为当前帧的值
                self.previous_turn_angle = turn_angle
        else:
            # 初始化 previous_turn_angle 为首次计算出的 turn_angle
            self.previous_turn_angle = turn_angle
