...
# 将轨迹点调整至与障碍物保持4米的最小距离，并判断偏移方向
def avoid_obstacle(trajectory_points, obstacles, obstacles_distance, safe_distance=4, num_adjacent=10, distance_threshold=5):
    xy_data = np.array([latlon_to_utm(item[0],item[1]) for item in trajectory_points])
    new_xy_data = xy_data.astype(np.float64)  # 确保 new_trajectory 为 float64 类型
    adjusted_points = np.zeros(len(xy_data), dtype=bool)  # 用于记录哪些点已经调整过
    
    # 1. 将距离较近的障碍物分为一组
    obstacle_groups,obstacle_idxs = group_nearby_obstacles(obstacles, distance_threshold)
    # print(obstacle_groups)
    for group, obstacle_idx in zip(obstacle_groups, obstacle_idxs):
        # 2. 计算障碍物组的中心位置（质心）
        group_center = calculate_group_center(group)
        group_direction = None
        group_direction = get_offset_direction(obstacles_distance, obstacle_idx)
        for one_obstacle in group:
            print("one_obstacle:",one_obstacle)
            # 3. 对每个轨迹点，判断它与障碍物组的中心位置的距离
            min_dist = float('inf')
            closest_point_idx = -1
            for i, point in enumerate(xy_data):
                dist = calculate_distance(point, one_obstacle)
                if dist < min_dist:
                    min_dist = dist
                    closest_point_idx = i
            ...
            if min_dist < safe_distance:
                offset_distance = safe_distance - calculate_distance(xy_data[closest_point_idx], one_obstacle)
                print(f"offset_distance: {offset_distance}")
                    # print(f"direction: {group_direction}")
                # 6. 确定受影响的轨迹点范围：当前点前后num_adjacent个点
                start_index = max(0, closest_point_idx - num_adjacent)  # 确保不越界
                end_index = min(len(trajectory_points), closest_point_idx + num_adjacent + 2)  # 确保不越界
                
                # 7. 偏移受影响的轨迹点，并标记已经调整过的点
                for j in range(start_index, end_index):
                    if not adjusted_points[j]:  # 如果该点没有被调整过
                        new_xy_data[j] += group_direction * np.array([offset_distance, 0])  # 沿X轴偏移4米（假设偏移方向沿X轴）
                        adjusted_points[j] = False  # 标记该点已经调整过
    adjusted_xy_trajectory = np.array([utm_to_latlon(item[0],item[1]) for item in new_xy_data])
    adjusted_xyheading_trajectory = np.array([(item[0],item[1],original_item[2]) for item,original_item in zip(adjusted_xy_trajectory,trajectory_points)])
    return adjusted_xyheading_trajectory
...
def obstacle_position(lat0, lon0, heading, x, y):
    ...
    # print(f"车辆的UTM坐标：X：{x0}, Y：{y0}")
    # print(f"车辆的航向角：{theta}")
              
    # 计算障碍物的 UTM 坐标
    obstacle_x = x0 + delta_E
    obstacle_y = y0 + delta_N
    # obstacle_x = x0 + delta_N
    # obstacle_y = y0 + delta_E
...
class VehicleTrajectoryFollower:
    def __init__(self, trajectory_csv,target_index=0):
        """
        初始化，读取轨迹点
        :param trajectory_csv: 包含轨迹点的CSV文件路径，轨迹点格式为[经度, 纬度, 航向角度]
        """
        self.original_trajectory = read_csv(trajectory_csv)
        self.trajectory_points = self.original_trajectory.copy()
        
        self.closest_index = 0
        self.wheelbase = 2.85
        self.offset_target_index = target_index
        self.target_index = 0
        self.obstacle_detected = True  # 标记是否检测到障碍物
    ...
    def init_closest_index(self,current_position, current_heading):
        ...
        for i, (lon, lat, _) in enumerate(self.trajectory_points):  # 经度在前，纬度在后
            distance = self.calculate_distance(front_lat, front_lon, lat, lon)
            if distance < min_distance:
                min_distance = distance
                closest_index = i
        self.closest_index =  closest_index+self.closest_index
    ...
    def find_closest_point_index(self, current_lat, current_lon):
        ...
        if self.closest_index == 0:
            max_bound = len(self.trajectory_points)-1
        else:
            max_bound = 200
        for i, (lon, lat, _) in enumerate(self.trajectory_points[self.closest_index:self.closest_index+max_bound]):  # 经度在前，纬度在后
    ...
    def calculate_turn_angle(self, current_position, current_heading):
        ...
        target_index = min(self.closest_index + self.offset_target_index, len(self.trajectory_points) - 1)  # 防止超出范围
    ...
    # 计算期望速度和加速度
    def calculate_speedAndacc(self,turn_angle,current_position, current_speed ,points_num_threshold = 20):
        if current_speed < 1:
            speed = 15
            acc = 0
            return  speed,acc
        ...
        next_lon, next_lat, _ = self.trajectory_points[self.closest_index+self.far_index]  # 注意经纬度顺序
        ...
        if far_turn_angle > 180:
            far_turn_angle -= 360        
        #映射到方向盘转角
        if far_turn_angle*WHEEL_FACTOR>380:
            far_turn_angle = 380
        elif far_turn_angle*WHEEL_FACTOR<-380:
            far_turn_angle = -380
        else:
            far_turn_angle = far_turn_angle*WHEEL_FACTOR    

        if abs(far_turn_angle) >= 30:
            print("=_="*30)
            if current_speed >= 6:
                speed = 6
                acc = -1
            ...
            speed = 15
            acc = 0
        # # 如果目标点在中终点20个点以内了，速度置为0
        # if len(self.trajectory_points)-self.target_index  <= points_num_threshold:
        #     speed = 0
        #     acc = -1
        return speed,acc    


    def adjust_trajectory_for_obstacle(self, obstacle_x, obstacle_y, offset_distance):
        trajectory = self.trajectory_points
        closest_index = self.closest_index
        direction = 'left' if obstacle_y > 0 else 'right'
        num_points_to_adjust = max(1, int(2 * obstacle_x + 0))
        num_points_to_adjust = min(num_points_to_adjust, len(trajectory) - closest_index)
        adjusted_trajectory = trajectory.copy()

        for i in range(closest_index+5, closest_index + num_points_to_adjust+5):
            if i >= len(adjusted_trajectory):
                break
            lon, lat, heading = adjusted_trajectory[i]
            x, y = latlon_to_utm(lon, lat)
            new_x, new_y = offset_point_utm(x, y, heading, offset_distance, direction)
            new_lon, new_lat = utm_to_latlon(new_x, new_y)
            adjusted_trajectory[i] = [new_lon, new_lat, heading]
            # print(lon,lat,new_lon,new_lat)
        self.trajectory_points = adjusted_trajectory
...
def start_main(shared_data,can_use,follower,filter):
    ...
    global mod_AE
    global manual_triggered
    obstacles_memoryBank = [] # 存儲障礙物utm坐標
    obstacles_list = []
    ...
    while True:
        ...
        if mod_AE == 1 and mod_666 == 1:
            if can_use.ego_lon is not None and can_use.ego_lat is not None:
                ...
                obstacle_y = None
                x,y = None, None
                if follower.obstacle_detected:
                    with shared_data["lock"]:
                        if shared_data["frame"]['perception_result'] is not None:
                            last_percepton_frame = shared_data["frame"]['perception_result']
                            for one_obstacle in last_percepton_frame:
                                obstacle_x = one_obstacle[2]
                                obstacle_y = one_obstacle[0]
                                if obstacle_x is not None and obstacle_y is not None and abs(obstacle_y) < 4 and abs(obstacle_x) < 10:
                                    obstacle_x_utm, obstacle_y_utm = obstacle_position(
                                        can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw, obstacle_x, obstacle_y)
                                    if should_ignore_obstacle([obstacle_x_utm, obstacle_y_utm], obstacles_memoryBank, distance_threshold=1):
                                            # print("ignore")
                                            continue
                                    else:
                                        ...
                                        obstacles_list.append([obstacle_x, obstacle_y])
                                        adjusted_result_trajectory = avoid_obstacle(follower.trajectory_points, np.array(obstacles_memoryBank), obstacles_list, safe_distance=5)
                                        
                                        follower.trajectory_points = adjusted_result_trajectory
                                    # follower.adjust_trajectory_for_obstacl.e(obstacle_x, obstacle_y, 5-abs(obstacle_y))
                                    # follower.obstacle_detected=False
                # print(can_use.ego_v)
                turn_angle = follower.calculate_turn_angle((can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw), can_use.ego_yaw)
                # desired_speed, desired_acc = 10, 0
                desired_speed, desired_acc = follower.calculate_speedAndacc(turn_angle, (can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw), can_use.ego_v)
...
# 使用锁来读取共享数据
        ...
        if last_control_frame != None:
            # pass
            # print(last_control_frame)
            can_use.publish_planner_ation(action = last_control_frame, id=0x666, action_type="acc", mod=1, enable=1)
...
def main():
    # 使用示例
    camera_index = 0
    trajectory_csv = '/home/nvidia/vcii/follow_trajectory/collect_trajectory/processed_haima-v2-1111.csv'
    # 10
    follower = VehicleTrajectoryFollower(trajectory_csv,target_index=5)
    # 创建过滤器实例
