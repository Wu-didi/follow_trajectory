...
class VehicleTrajectoryFollower:
    ...
    def adjust_trajectory_for_obstacle(self, obstacle_x, obstacle_y, offset_distance):
        ...
        for i in range(closest_index+0, closest_index + num_points_to_adjust+0):
                    ...
                    with shared_data["lock"]:
                        if shared_data["frame"]['perception_result'] is not None:
                            ...
                            for one_obstacle in last_percepton_frame:
                                ...
                                if follower.closest_index ==0:
                                    continue
                                else:
                                    if obstacle_x is not None and obstacle_y is not None and abs(obstacle_y) < 3 and abs(obstacle_x) < 15:
                                        obstacle_x_utm, obstacle_y_utm = obstacle_position(can_use.ego_lat, can_use.ego_lon, can_use.ego_yaw, obstacle_x, obstacle_y)
                                    
                                        if should_ignore_obstacle([obstacle_x_utm, obstacle_y_utm], obstacles_memoryBank, distance_threshold=1.0):
                                            # print("ignore")
                                            continue
                                        else:
        
                                            print("oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo")
                                            follower.adjust_trajectory_for_obstacle(obstacle_x, obstacle_y, 5-abs(obstacle_y))
                                            obstacles_memoryBank.append([obstacle_x_utm, obstacle_y_utm])
                        # else:
                        #             follower.reset_trajectory()
