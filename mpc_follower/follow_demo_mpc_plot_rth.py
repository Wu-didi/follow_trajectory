...
# mpc parameters
R = np.diag([0.0 , 0.1])  # input cost matrix
Rd = np.diag([0.0, 0.0])  # input difference cost matrix
Q = np.diag([10000000, 10000000, 1, 1])  # state cost matrix
...
def normalize_angle_rad(angle):
    """
    将角度规范化，保持符号，使其位于 [-π, π) 范围内。

    参数:
        angle (float): 输入的角度（弧度）
...
class VehicleTrajectoryFollower:
    ...
    def calculate_turn_angle(self, ego_state, ego_yaw, ego_v):
        ...
        # print("delta reference:", self.dref[1])
        # print("o delt",odelta)
        # print("===================================== cost output=================================")
        
        # print("x_cost =", sum(self.xref[0] - ox))
        # print("y_cost =", sum(self.xref[1] - oy))
        # print("yaw_cost = ", sum(self.xref[3] - oyaw))
        # print("delta_cost = ", sum(self.dref[1][:-1] - odelta))
        # print('------------------------------------')
        
        plt.figure()
        plt.scatter(ox, oy, c='r', marker='x')
        plt.plot(self.cx, self.cy, c='g')
        plt.scatter(self.xref[0],self.xref[1],c='b', alpha=0.5)
        plt.axis("equal")
        ...
        if odelta is not None:
            ...
            di = normalize_angle_rad(di)
            print("nomralized di: ", di)
            ...
            print("filter angle :", filtered_angle)
            # 更新MPC控制器的上一轮控制输入
            self.oa = oa
            self.odelta = odelta
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, ck, sp, dl, pind):
        ...
        xref[2, 0] = sp[ind]
        xref[3, 0] = cyaw[ind]
        dref[1, 0] = math.atan2(WB * ck[ind], 1)  # steer operational point should be 0
        # dref[0, 0] = self.calculate_reference_steer(state.yaw, cyaw[ind])
        ...
        for i in range(T + 1):
            ...
            # if (ind + i) < ncourse:
                ...
                xref[2, i] = sp[ind + i]
                xref[3, i] = cyaw[ind + i]
                dref[1, i] = math.atan2(WB * ck[ind + i], 1)
                # dref[0, i] = self.calculate_reference_steer(dref[0,i-1], cyaw[ind + dind])
            else:
                xref[0, i] = cx[ncourse - 1]
                xref[1, i] = cy[ncourse - 1]
                xref[2, i] = sp[ncourse - 1]
                xref[3, i] = cyaw[ncourse - 1]
                dref[1, i] = math.atan2(WB * ck[ncourse-1], 1)
    ...
    def calculate_reference_steer(self, dref_, ref_yaw):
        # 简单的参考转向角计算，可以根据需要调整
        # print(dref_)
        # print(ref_yaw)
        delta = pi_2_pi(ref_yaw - dref_)
        # 限制转向角在物理范围内
    ...
    def linear_mpc_control(self, xref, xbar, x0, dref):
        ...
        for t in range(T):
            ...
            if t != 0:
                cost += cvxpy.quad_form(x[:, t] - xref[:, t], Q)  # state cost function

            A, B, C = get_linear_model_matrix(
                xref[2, t], xref[3, t], dref[1, t])
...
class Can_use:
    def __init__(self, zone):
        ...
        # 用于平滑航向角
        self.previous_yaw = 3.13  # 初始 yaw 转换为弧度
    ...
    while True:
        ...
        if mod_AE == 1 and mod_666 == 1:
            ...
            if can_use.ego_x is not None and can_use.ego_y is not None:     
                ...
                print("=======mpc计算得到的:",turn_angle,"===========滤波后的：",filtered_angle)
...
def send_frame(shared_data,can_use,filter):
    last_frame = None
    global mod_AE
    # print("here===================================")
    while True:
        ...
        if last_frame != None:
        #    pass
            last_frame[1] = filter.update_speed(last_frame[1])
            
            # can_use.publish_planner_ation(action = last_frame, id=0x666, action_type="acc", mod=1, enable=1)
            
            logging.info(f"send result: {last_frame}, time: {time.time()}")
            # print("send result : ", last_frame)
            # print("send time cost: ",time.time()-t0)
...
def main():
    ...
    send_thread = threading.Thread(target=send_frame, args=(shared_data,can_use,filter))
    # 键盘监听线程
