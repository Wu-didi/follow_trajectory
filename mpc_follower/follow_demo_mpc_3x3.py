...
Q = np.diag([10.0, 10.0, 10])  # state cost matrix
...
def get_linear_model_matrix(v, phi, delta):
    A = np.matrix([
        [1.0, 0.0, -v*DT*math.sin(phi)],
        [0.0, 1.0, v*DT*math.cos(phi)],
        [0.0, 0.0, 1.0]])
    B = np.matrix([
        [DT*math.cos(phi), 0],
        [DT*math.sin(phi), 0],
        [DT*math.tan(ref_delta)/WB, v*DT /(WB*math.cos(delta)*math.cos(delta))]
    ])
    C = np.eye(3)
    return A, B, C
...
class VehicleTrajectoryFollower:
    ...
    def calculate_turn_angle(self, ego_state, ego_yaw, ego_v):
        ...
        plt.scatter(self.cx[self.target_ind], slef.cy[self.target_ind], c='b')
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, ck, sp, dl, pind):
        ...
        ind, _ = self.calc_nearest_index(state, cx, cy, cyaw, 0)
        ...
        for i in range(T + 1):
            ...
            if (ind + i) < ncourse:
                ...
                xref[3, i] = cyaw[ind + i]
                # dref[0, i] = 0.0
                dref[0, i] = self.calculate_reference_steer(state, cyaw[ind + i])
            else:
    ...
    def linear_mpc_control(self, xref, xbar, x0, dref):
        ...
        for t in range(T):
            cost += cvxpy.quad_form(u[:, t]-dref[:,t], R)
            if t != 0:
