...
class VehicleTrajectoryFollower:
    ...
    def is_car_at_endpoint(car_x, car_y, car_yaw, end_position):
        def is_within_threshold(value, target, threshold):  
            return abs(value - target) <= threshold    
        # 计算位置接近程度  
        distance = math.sqrt((car_x - end_position[0]) ** 2 + (car_y- end_position[1]) ** 2)  
        position_close = is_within_threshold(distance, 0, 1)  
        
        # 计算航向角接近程度  
        heading_close = is_within_threshold(car_yaw, end_position[2], 10)  

    
        # 同时满足位置和航向角接近条件时，认为主车到达终点  
        return position_close and heading_close  
...
def start_main(shared_data,can_use,follower):
    ...
    ego_record_path = formatted_time + "ego" + ".csv"
    global stop_record
    while True:
        frames += 1
        ego_info = []
        ...
        if can_use.ego_lon is not None and can_use.ego_lat is not None:
            ...
            if stop_record:
                ...
                with open(ego_record_path, mode='w', newline='') as file:  
                    writer = csv.writer(file)  
                    writer.writerows(ego_info_list)
        else:
            print("主车定位丢失")
            new_frame = [-2, 0]
...
def main():
    # 使用示例
    trajectory_csv = '/home/nvidia/vcii/follow_trajectory/processed_syzx_v2_checked.csv'
